---
aliases:
title: TOC
date: 2026-05-02
tags:
  - wiki
description: ""
draft: false
---

# Table of Contents

Catalog of wiki pages by category.

## Overview
- [[Wiki/Overview|Overview]] | High-level synthesis.

## Entities
- [[Wiki/Entities/Obsidian|Obsidian]] | Note-taking IDE for wiki.
- [[Wiki/Entities/Andrej Karpathy|Andrej Karpathy]] | LLM Wiki author.
- [[Wiki/Entities/Vannevar Bush|Vannevar Bush]] | Memex inspiration.
- [[Wiki/Entities/Memex|Memex]] | Vannevar Bush's associative knowledge device (1945).
- [[Wiki/Entities/Eureka Labs|Eureka Labs]] | Karpathy's AI ed startup.
- [[Wiki/Entities/MenuGen|MenuGen]] | Karpathy's menu viz app.
- [[Wiki/Entities/Maddie|Maddie]] | AI educator (ex-Google/Amazon).
- [[Wiki/Entities/OpenClaw|OpenClaw]] | Viral open-source agent.
- [[Wiki/Entities/Peter Steinberger|Peter Steinberger]] | OpenClaw creator (OpenAI).
- [[Wiki/Entities/Anthropic|Anthropic]] | MCP creators.
- [[Wiki/Entities/Adobe Firefly|Adobe Firefly]] | Creative genAI.
- [[Wiki/Entities/BBVA|BBVA]] | 4k+ Custom GPTs.
- [[Wiki/Entities/CMA CGM|CMA CGM]] | MAIA (Mistral).
- [[Wiki/Entities/Intercom|Intercom]] | Fin agent.
- [[Wiki/Entities/L’Oréal|L’Oréal]] | Firefly CREAITECH.
- [[Wiki/Entities/Mistral AI|Mistral AI]] | Efficient LLMs.
- [[Wiki/Entities/Shaya Ike Hassan|Shaya Ike Hassan]] | AI industry blogger.
- [[Wiki/Entities/Snowflake|Snowflake]] | AI databases for agents.

## Concepts
- [[Wiki/Concepts/LLM-Wiki|LLM-Wiki]] | Pattern for LLM knowledge bases.
- [[Wiki/Concepts/RAG|RAG]] | Query-time retrieval (non-persistent).
- [[Wiki/Concepts/Persistent Wiki|Persistent Wiki]] | Compounding wiki artifact.
- [[Wiki/Concepts/Projects|Projects]] | Trust-based AI coding.
- [[Wiki/Concepts/Agentic Engineering|Agentic Engineering]] | Pro agent coordination.
- [[Wiki/Concepts/Software 3.0|Software 3.0]] | Prompting LLMs.
- [[Wiki/Concepts/LLMs|LLMs]] | Large Language Models (next-token prediction).
- [[Wiki/Concepts/Tokens|Tokens]] | LLM text units (words/subwords).
- [[Wiki/Concepts/Context Window|Context Window]] | Max tokens processed (model memory).
- [[Wiki/Concepts/AI Agents|AI Agents]] | Autonomous perceive-reason-act loops.
- [[Wiki/Concepts/MCP|MCP]] | Model Context Protocol (AI USB).
- [[Wiki/Concepts/Fine-tuning|Fine-tuning]] | Adapt LLM behavior/style.
- [[Wiki/Concepts/Context Engineering|Context Engineering]] | Curate full LLM context.
- [[Wiki/Concepts/Reasoning Models|Reasoning Models]] | Chain-of-thought (e.g. o1).
- [[Wiki/Concepts/Multimodal AI|Multimodal AI]] | Text+image/audio/video.
- [[Wiki/Concepts/Mixture of Experts|Mixture of Experts]] | Efficient expert routing (MOE).
- [[Wiki/Concepts/Agentic Commerce|Agentic Commerce]] | Agentic commerce shift.
- [[Wiki/Concepts/EU AI Act|EU AI Act]] | 2026 regulations.
- [[Wiki/Concepts/Great Divergence|Great Divergence]] | Vertical AI adoption split.
- [[Wiki/Concepts/Inference Economics|Inference Economics]] | Deployment economics.
- [[Wiki/Concepts/Multi-Agent Systems|Multi-Agent Systems]] | Agent teams.
- [[Wiki/Concepts/Physical AI|Physical AI]] | AI + robotics.

## Sources
- [[Wiki/Sources/The Kaparthy LLM Wiki|The Kaparthy LLM Wiki]] | Karpathy's foundational gist.
- [[Wiki/Sources/Andrej Karpathy - AI Ascent 2026|Andrej Karpathy - AI Ascent 2026]] | Talk on vibe coding, agentic engineering, Software 3.0.
- [[Wiki/Sources/10-AI-Concepts|10-AI-Concepts]] | Maddie: 10 essential AI concepts (2026).
- [[Wiki/Sources/Ways-to-Use-AI-in-Daily-Life|Ways-to-Use-AI-in-Daily-Life]] | 101 practical AI use cases across daily life areas (productivity, work, learning, etc.).
- [[Wiki/Sources/The Artificial Intelligence Industry 2026|The Artificial Intelligence Industry 2026]] | 2026 AI industry: Inference Economics, $2T spending, agentic shift, EU AI Act.

## Comparisons & Analyses
- [[Wiki/Vibe-Coding-vs-Agentic-Engineering|Vibe-Coding-vs-Agentic-Engineering]] | Synthesis table: Vibe Coding (democratizing floor) vs Agentic Engineering (professional ceiling).
